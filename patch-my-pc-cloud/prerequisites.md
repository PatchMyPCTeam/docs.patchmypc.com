...
<p>Patch My PC Cloud supports authentication through Microsoft Entra ID only. Direct SSO with third-party identity providers such as Okta or PingFederate is not currently supported.</p>
<p>Patch My PC Cloud does not expose SAML or OIDC endpoints for customer-managed third-party IdP integrations. As a result, third-party IdPs cannot federate directly with Patch My PC Cloud, and Patch My PC Cloud cannot directly consume or validate assertions, tokens, or MFA claims issued by those providers.</p>
</blockquote>
